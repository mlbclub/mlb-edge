...
SIM_FEATURES = [
    "diff_elo_pre", "diff_days_rest", "diff_games_last7", "diff_games_last4",
    "diff_win_r10", "diff_win_r20", "diff_run_diff_r10", "diff_run_diff_r20",
    "diff_win_trend_5v20", "diff_run_diff_trend_5v20",
    "diff_bat_ops_r10", "diff_bat_ops_r20", "diff_bat_ops_trend_5v20",
    "diff_bat_hr_rate_r10",
    "diff_venue_win_r20", "diff_venue_run_diff_r20", "diff_venue_bat_ops_r20",
    "diff_bullpen_era_r10", "diff_bullpen_whip_r10",
    "diff_bullpen_era_trend_5v20", "diff_bullpen_whip_trend_5v20",
    "diff_bullpen_pitches_usage_1", "diff_bullpen_pitches_usage_2",
    ...
    "diff_starter_vs_opp_starts", "diff_starter_era_vs_opp",
    "diff_starter_whip_vs_opp", "diff_starter_k9_vs_opp",
]
BASE_COLS = [
...
def load_similarity_history():
    ...
    if HIST_ODDS_FILE.exists():
        ...
        if "game_pk" in odds_cols:
            od = pd.read_csv(HIST_ODDS_FILE, usecols=odds_cols).drop_duplicates("game_pk", keep="last")
...
def historical_similarity(row: dict, game_dt, home_team_id: int, away_team_id: int,
                          current_odds: dict | None = None, max_neighbors: int = 100) -> dict:
    ...
    for c in usable:
        ...
        d2 += np.square(np.clip(z, -5.0, 5.0))
        dims += 1
    ...
    if odds_available and cur_mp is not None and "home_market_novig" in past.columns:
        ...
        penalty[mask] = np.square((vals[mask] - float(cur_mp)) / 0.055)
        penalty[~mask] = 2.25
        d2 += penalty
        dims += 1
    ...
    if cur_total is not None and "total_line" in past.columns:
        ...
        mask = np.isfinite(vals)
        penalty = np.zeros(len(past), dtype=float)
        penalty[mask] = np.square((vals[mask] - float(cur_total)) / 0.65)
        penalty[~mask] = 1.5
        d2 += penalty
        dims += 1
    ...
    analog_raw = _weighted_mean(pd.to_numeric(neigh["home_win"], errors="coerce").to_numpy(float), weights)
    eff_n = _effective_n(weights)
    analog_home = _shrink(analog_raw, eff_n, prior=0.5, prior_n=20.0)
    ...
    h2h = past[h2h_mask].sort_values("game_date").tail(24)
    h2h_prob, h2h_n = None, int(len(h2h))
    if h2h_n:
        vals = np.where(
        ...
        recency_w = np.linspace(0.55, 1.0, h2h_n)
        h2h_prob = _shrink(_weighted_mean(vals, recency_w), h2h_n, prior=0.5, prior_n=14.0)

    if analog_home is not None and h2h_prob is not None:
        h2h_weight = min(0.16, 0.02 * h2h_n)
    ...
    if cur_total is not None:
        ...
        if valid.any():
            line = float(cur_total)
            push = np.isclose(totals, line) if line.is_integer() else np.zeros(len(totals), dtype=bool)
            wv = weights[valid]
            push_valid = push[valid]
            p_push = _weighted_mean(push_valid.astype(float), wv) or 0.0
            nonpush_w = wv * (~push_valid).astype(float)
            if nonpush_w.sum() > 0:
                ne = _effective_n(nonpush_w)
                over_prob = _shrink(_weighted_mean((totals[valid] > line).astype(float), nonpush_w), ne, prior=0.5, prior_n=20.0)
                under_prob = _shrink(_weighted_mean((totals[valid] < line).astype(float), nonpush_w), ne, prior=0.5, prior_n=20.0)
    ...
    return {
        ...
        "over_prob": over_prob, "under_prob": under_prob, "push_prob": push_prob,
        "neighbors": int(take), "effective_n": float(eff_n),
        "h2h_n": h2h_n, "h2h_home_prob": h2h_prob,
